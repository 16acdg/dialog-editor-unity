...
public class DialogEditorWindow : EditorWindow
{
    ...
    private Conversation selectedCharacter;  // Character's dialog that is being viewed in the editor window

    // Removed this capability for first version
    ...
    /// </summary>
    /// <param name="character">Character with a Conversation component.</param>
    public void ShowWindowForCharacter(Conversation character)
    {
        if (character != null)
        {
    ...
    private void DisplayNodeInfo()
    {
        ...
        if (selectedNodeIndex != END_NODE_INDEX)
        {
            ...
            {
                // Text area for entering the response text:
                ...
                GUI.Label(new Rect(175 * i, 80, 150, 20), connectionDestinationString);
                // Text field for naming a function to invoke
                nodes[selectedNodeIndex].content.responses[i].triggerFunctionName = GUI.TextField(new Rect(175 * i, 60, 125, 20), nodes[selectedNodeIndex].content.responses[i].triggerFunctionName);
                // Delete dialog path on button press:
    ...
    private void RemoveNodeAtIndex(int index)
    {
        ...
        for (int i = 0; i < nodes.Count; i++)
        {
            List<DialogResponse> dialogResponses = nodes[i].content.responses;
            for (int j = 0; j < dialogResponses.Count; j++)
            {
                if (dialogResponses[j].nextSentence == index)
                {
